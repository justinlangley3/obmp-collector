...
/**
 * Parse the kafka configuration generically into a map
 *
 * \param [in] node     Reference to the yaml NODE
 */
...
/**
 * Parses the Kafka brokers setting
 *
 * \param [in] node    Reference to the yaml NODE
 */
void Config::parseBrokers(const YAML::Node &node) {
    std::ostringstream brokersStream;

    if (node.IsSequence()) {
        for (size_t i = 0; i < node.size(); ++i) {
            if (i > 0) brokersStream << ",";
            brokersStream << node[i].as<std::string>();
        }
...
/**
 * Returns true if configuration key contains sensitive info
 *
 * sensitive keys:
 ...
 */
bool Config::isSensitive(const std::string& key) {
...
/**
 * Prints the Kafka configuration properties
 *
 */
void Config::printKafkaProperties() {
    ...
    for (const auto& entry : kafka_config_map) {
        const std::string& key = entry.first;
        const std::string& value = entry.second;
        std::string valueToPrint = isSensitive(key) ? "********" : value;
        std::cout << "  " << key << ": \"" << valueToPrint << "\"" << std::endl;
    }
}
